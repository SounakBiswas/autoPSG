...
IfFirstOrderDEq2D[HoldPattern[Equation[Times[ Inv[F[B_]][{x1_,y1_}], F[A_][{x2_,y2_}]],rhs_]]]:=
With[{k1=x2-x1, k2=y2-y1},
If[((NumericQ[k1]&&Abs[k1]<=1) && (NumericQ[k2] && Abs[k2]<= 1 )) &&Not[(k1==0 && k2==0)]&&(Abs[k1]+Abs[k2]==1)&&SameQ[A,B],
True,
False]];
...
p//.{x->b,y->a}];
If[k1==0 && Abs[k2]==1,
 {Rule@@{F[A][{a_,b_}], F[A][{a,0}]expr^b}},
If[Abs[k1]==1 && k2==0,
{Rule@@{F[A][{a_,b_}], F[A][{0,b}]expr^a}},
If[Abs[k1]==1 && Abs[k2]==1,
{Rule@@{F[A][{a_,b_}], F[A][{0,0}]expr^(a+b)}}]]]];
...
solveByFSubstitutions[FSubstAssoc_,HoldPattern[eq:Equation[Times[(F1:F[A_][coord1_]|Inv[F[A_]][coord1_]),(F2:F[B_][coord2_]|Inv[F[B_]][coord2_])],rhs_]]]:=Module[{newList},
